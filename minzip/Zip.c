...
//      and stop after the first non-match.
    ...
    bool ok = true; // eddie@motorola 'ok' used as boolean below (IKMAP-3991)
...
#define UNZIP_FILEMODE 0644
        if (pEntry->fileName[pEntry->fileNameLen-1] == '/') {
            ...
            if (!(flags & MZ_EXTRACT_FILES_ONLY) && mzIsZipEntrySymlink(pEntry)) {
                ...
                ok = mzExtractZipEntryToFile(pArchive, pEntry, fd); // eddie@motorola we're better not define new 'ok' here (IKMAP-3991)
                close(fd);
                if (!ok) {
