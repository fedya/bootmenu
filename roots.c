...
static RootInfo g_roots[] = {
    ...
    { "/cache/", g_mtd_device, NULL, "cache", "/cache", "yaffs2" }, // eddie@motorola.com ported from morrison
    ...
    { "/sdcard/", "/dev/block/mmcblk0p1", "/dev/block/mmcblk0", NULL, "/sdcard", "vfat" },  // eddie@motorola.com ported from morrison
...
get_root_info_for_path(const char *root_path)
{
     ...
     */
    c = root_path;
    if (*c != 0) c++; // eddie@motorola.com ported from morrison
    while (*c != '\0' && *c != ':' && *c != '/') { // eddie@motorola.com ported from morrison
        c++;
    }
    if (*c == '\0') {
        return NULL;
